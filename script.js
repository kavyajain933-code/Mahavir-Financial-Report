...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.electronAPI) {
        ...
        window.electronAPI.onUpdateDownloaded(() => {
            ...
            document.getElementById('update_message').textContent = 'Restart now to install?';
...
function navigate(pageId) {
    ...
    if(pageId === 'add_stock' || pageId === 'check_inventory') { renderCategoryDropdowns(); if(pageId==='check_inventory') renderInventory(); }
    if(pageId === 'record_sale' && document.getElementById('barcode_scan_input')) document.getElementById('barcode_scan_input').focus();
    if(pageId === 'sales_details') { document.getElementById('sales_start_date').value = today; document.getElementById('sales_end_date').value = today; renderSalesDetails(); }
    if(pageId === 'repair_details') { document.getElementById('repair_start_date').value = today; document.getElementById('repair_end_date').value = today; renderRepairDetails(); }
    if(pageId === 'recharge_details') { document.getElementById('recharge_start_date').value = today; document.getElementById('recharge_end_date').value = today; renderRechargeDetails(); }
    if(pageId === 'reports') { document.getElementById('report_month_year').value = currentYear; document.getElementById('report_year').value = currentYear; document.getElementById('ai_summary_container').classList.add('hidden'); }
...
function renderHomepage() {
    ...
    if (lowStockItems.length > 0) lowStockItems.forEach(i => lowStockList.innerHTML += `<p class="text-gray-300 p-2 bg-gray-700 rounded-md mb-2">${i.name} - <span class="font-bold">${i.quantity} left</span></p>`);
    else lowStockList.innerHTML = '<p class="text-gray-400">No low stock items. Great job!</p>';
}

function updateClock() { document.getElementById('clock').textContent = new Date().toLocaleTimeString(); }
function showStatus(id, msg, isErr = false) {
    const el = document.getElementById(id); if(!el) return;
    el.textContent = msg; el.className = `h-6 text-center ${isErr ? 'text-red-400' : 'text-green-400'}`;
...
function removeCategory(c) { showConfirmModal(`Remove "${c}"?`, async () => { categories = categories.filter(cat => cat !== c); await saveMainData(); renderCategoryList(); renderCategoryDropdowns(); }); }
function removeAllCategories() { showConfirmModal("Remove all?", async () => { categories = ["Other"]; await saveMainData(); renderCategoryList(); renderCategoryDropdowns(); }); }
...
function renderCategoryList() { document.getElementById('category_list').innerHTML = categories.map(c => `<div class="flex justify-between items-center bg-gray-700 p-2 rounded"><span>${c}</span><button onclick="removeCategory('${c}')" class="text-red-400 font-bold">X</button></div>`).join(''); }
...
function openAdjustQuantityModal(id) { const i = stock.find(x=>x.id===id); document.getElementById('adjust_product_name').textContent=i.name; document.getElementById('adjust_current_quantity').textContent=`Qty: ${i.quantity}`; document.getElementById('quantity_to_remove').value=''; document.getElementById('adjust_quantity_confirm').onclick=()=>adjustQuantity(id); document.getElementById('adjust_quantity_modal').classList.add('flex'); }
function closeAdjustQuantityModal() { document.getElementById('adjust_quantity_modal').classList.remove('flex'); }
function openEditStockModal(id) { editingItemId=id; const i = stock.find(x=>x.id===id); renderCategoryDropdowns(); document.getElementById('edit_product_name').value=i.name; document.getElementById('edit_barcode').value=i.barcode; document.getElementById('edit_product_category').value=i.category; document.getElementById('edit_purchase_price').value=i.purchasePrice; document.getElementById('edit_stock_modal').classList.add('flex'); }
...
function openSellPriceModal(p) { productForPriceEntry=p; document.getElementById('sell_price_product_name').textContent=p.name; document.getElementById('sell_price_input').value=Math.round(p.purchasePrice*1.5); document.getElementById('sell_price_modal').classList.add('flex'); document.getElementById('sell_price_input').focus(); }
...
function confirmSellPrice() { const sp = parseFloat(document.getElementById('sell_price_input').value); if(isNaN(sp)||sp<=0) return; currentSale.push({...productForPriceEntry, sellPrice:sp, sellQuantity:1, totalProfit:(sp-productForPriceEntry.purchasePrice)}); renderCart(); closeSellPriceModal(); }
...
function selectProductForSale(id) { selectedProductForSale=stock.find(i=>i.id===id); document.getElementById('select_product_button').textContent=selectedProductForSale.name; document.getElementById('available_stock').textContent=`Avail: ${selectedProductForSale.quantity}`; closeProductSearch(); }
...
function renderCart() {
    ...
    document.getElementById('cart_total_amount').textContent=`Total: ${formatCurrency(currentSale.reduce((a,b)=>a+(b.sellPrice*b.sellQuantity),0))}`;
}
...
async function saveApiKey() { apiKey = document.getElementById('api_key_input').value.trim(); await saveMainData(); showStatus('api_key_status', 'Saved.'); }
function checkForUpdate() { showStatus('update_check_status', 'Checking...', false); if (window.electronAPI) window.electronAPI.checkForUpdate(); }
async function getAIInsights() {
    if (!apiKey) { showStatus('report_status', 'No API Key.', true); return; }
    document.getElementById('ai_summary_container').classList.remove('hidden'); document.getElementById('ai_summary_content').innerHTML = 'Analyzing...';
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${apiKey}`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ contents: [{ parts: [{ text: "Analyze my shop performance based on sales." }] }] }) });
        const result = await response.json(); document.getElementById('ai_summary_content').innerHTML = result.candidates[0].content.parts[0].text;
    } catch (e) { document.getElementById('ai_summary_content').innerHTML = 'Error.'; }
}
function promptResetAllData() { showConfirmModal("DELETE ALL DATA?", async () => { stock=[]; categories=["Other"]; apiKey=''; await saveMainData(); }); }
function saveBackupToFile() { const blob = new Blob([JSON.stringify({stock,salesLog}, null, 2)], {type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='backup.json'; a.click(); }
function getCustomAIInsight() { /* (Keep existing logic if needed or empty) */ }
function generatePDFReport() { /* (Keep existing logic) */ }